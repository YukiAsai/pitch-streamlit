...
# 🛠 デバッグ：このサービスアカウントから見えるスプレッドシート一覧を表示
st.title("⚾ 一球データ入力アプリ（Google Sheets連携・デバッグモード）")

try:
    spreadsheet_list = client.openall()
    titles = [s.title for s in spreadsheet_list]
    st.write("📄 現在このサービスアカウントから見えるスプレッドシート一覧：")
    st.code(titles)

    # ✅ 本来の処理（見える場合だけ実行する）
    if "Pitch_Data_2025" in titles:
        sheet = client.open("Pitch_Data_2025").worksheet("input")
        st.success("✅ スプレッドシートにアクセス成功！")
    else:
        st.warning("⚠️ 'Pitch_Data_2025' という名前のスプレッドシートが見つかりません。")
        st.stop()

except Exception as e:
    st.error("❌ スプレッドシート一覧の取得に失敗しました。")
    st.code(str(e))
    st.stop()
...
creds = ServiceAccountCredentials.from_json_keyfile_dict(st.secrets["gcp_service_account"], scope)
client = gspread.authorize(creds)
sheet = client.open("Pitch_Data_2025").worksheet("input")  # スプレッドシート名をここで指定
